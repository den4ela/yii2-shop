...
$data = Product::find()->select('title')->where(['category_id' => 1])->all();

$select = [];

foreach ($data as $row) {
    array_push($select, $row['title']);
}
...
<script type="text/javascript">
	...
	<div class="banner banner1" style="background-image:url(<?php if($selected_category) echo $selected_category->getCategImgUrl(); else echo \backend\models\SiteSettings::getMainBannerUrl() ?>)">
		<div class="container">
			<h2>Отличные скидки <span><?php if($selected_category) echo $selected_category->title; else echo 'Электроника' ?></span></h2>
		</div>
	</div> 
				...
				<div class="col-md-4 w3ls_mobiles_grid_left">
								...
								<div class="panel-heading" role="tab" id="headingOne">
								  <h4 class="panel-title asd">
									<a class="pa_italic" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
									  <span class="glyphicon glyphicon-plus" aria-hidden="true"></span><i class="glyphicon glyphicon-minus" aria-hidden="true"></i>Каталог
									</a>
								  </h4>
								</div>
					...
					<div class="w3ls_mobiles_grid_left_grid">
						...
						<div class="w3ls_mobiles_grid_left_grid_sub">
							...
							<?=Html::beginForm(['catalog/list'],'get') ?>
								<ul>
									<li><input type="submit" name="less100" class="btn btn-link" value="менее $ 100"></li>
									...
									<li><input type="submit" name="more20k" class="btn btn-link" value="$ более 20k"></li>
								</ul>
							<?=Html::endForm(); ?>
							</div>
						</div>
				...
				<div class="col-md-8 w3ls_mobiles_grid_right">
					...
					<div class="w3ls_mobiles_grid_right_grid2">
						<div class="w3ls_mobiles_grid_right_grid2_left">
							<h3>Результаты: 0-<?=$pagination->totalCount ?></h3>
						</div>
						...
						<?=Html::beginForm(['catalog/list'], 'get') ?>
							...
							<?php switch($selected_sort){
								case 'default': echo '
								<option value="default" selected="selected">Обычный вид</option>
								<option value="newsort">По новизне</option>
								<option value="price_low">От дешевых</option>
								<option value="price_high">От дорогих</option>
								' ;
							    break;
                    ...
                    <input type="text" class="form-control" id="g_search" onkeyup="search_goods()" placeholder="Поиск.."><br><br>

                    <input type="hidden" id="category" value="<?php if($selected_category !== NULL ){echo $selected_category['title'];} else  echo 0; ?>">



					<div id="goods" class="w3ls_mobiles_grid_right_grid3">
						...
						<div class="col-md-4 agileinfo_new_products_grid agileinfo_new_products_grid_mobiles">
							<div class="agile_ecommerce_tab_left mobiles_grid">
								...
								<div class="simpleCart_shelfItem">
									...
									<button class="w3ls-cart"><?= Html::a('Добавить', ['cart/add', 'productId' => $product['id']]) ?></button>
								</div> 
